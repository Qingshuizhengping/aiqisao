...
// 读取头像 data URI（由 index.html header avatar 提供）
const AVATAR_URL = (document.querySelector('.header-avatar img') || {}).src || '/avatar.png';
...
function newChat() {
    ...
    ws.innerHTML = `
        <div class="welcome-icon"><img src="${AVATAR_URL}" class="welcome-avatar-img"></div>
        <h2>你好，我是AI七嫂</h2>
...
function switchChat(convId) {
    ...
    conv.messages.forEach(msg => {
        if (msg.role === 'user') {
            ...
            div.innerHTML = `<div class="qisao-avatar"><img src="${AVATAR_URL}" class="avatar-img"></div><div class="qisao-bubble">${msg.html}</div>`;
            chatContainer.appendChild(div);
        }
    });
    scrollToBottom();
...
function addQisaoMessage(html) {
    ...
    div.className = 'message message-qisao';
    div.innerHTML = `
        <div class="qisao-avatar"><img src="${AVATAR_URL}" class="avatar-img"></div>
...
function showTyping() {
    ...
    div.innerHTML = `
        <div class="qisao-avatar" style="width:32px;height:32px;flex-shrink:0;"><img src="${AVATAR_URL}" class="avatar-img"></div>
...
// 初始化应用
function initApp() {
    const _wa = document.querySelector(".welcome-avatar-img"); if (_wa) _wa.src = AVATAR_URL;
    loadConversations();
    renderSidebar();
